...
export function JumonBuilder(baseClass) {
  ...
  return class Jumon extends EventReceiver {
    ...
    _bindToDom(root, initialValue={}) {
      ...
      // shadow DOM does not have some attributes, so check is needed.
      const forEachValue = !!root.getAttribute ?
            root.getAttribute("jumon-foreach") : undefined;
      if (forEachValue) {
      ...
      for (const [key, func] of attributes) {
        ...
        if (root.matches && root.matches(key)) {
          func(root);
        }
      ...
      if (forEachValue) {
        root.setAttribute("jumon-foreach", forEachValue);
      }
    }
